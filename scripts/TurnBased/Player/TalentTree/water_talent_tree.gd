...
func _ready():
	for button in get_tree().get_nodes_in_group("water_talent_button"):
...
func _on_t_1b_1_toggled(toggled_on):
	...
	current_djinn.talent1_path = "res://scenes/TBScenes/Player/TalentTrees/WaterTalents/mana_adept.tscn"
...
func on_pressed(button):
	...
	if current_button.is_in_group("water_tier1") && current_button.button_pressed == true:
		enable_row(2)
	
	if current_button.is_in_group("water_tier2") && current_button.button_pressed == true:
		enable_row(3)
	
	if current_button.is_in_group("water_tier3") && current_button.button_pressed == true:
		enable_row(4)
			
	if current_button.is_in_group("water_tier4") && current_button.button_pressed == true:
		enable_row(5)
			
	if current_button.is_in_group("water_tier5") && current_button.button_pressed == true:
		enable_row(6)
			
	if current_button.is_in_group("water_tier6") && current_button.button_pressed == true:
		enable_row(7)
			
	if current_button.is_in_group("water_tier7") && current_button.button_pressed == true:
		enable_row(8)

func enable_row(row_number):
	match(row_number):
		1:
			for i in get_tree().get_nodes_in_group("water_tier1"):
				i.disabled = false
		2:
			for i in get_tree().get_nodes_in_group("water_tier2"):
				i.disabled = false
		3:
			for i in get_tree().get_nodes_in_group("water_tier3"):
				i.disabled = false
		4:
			for i in get_tree().get_nodes_in_group("water_tier4"):
				i.disabled = false
		5:
			for i in get_tree().get_nodes_in_group("water_tier5"):
				i.disabled = false
		6:
			for i in get_tree().get_nodes_in_group("water_tier6"):
				i.disabled = false
		7:
			for i in get_tree().get_nodes_in_group("water_tier7"):
				i.disabled = false
		8:
			for i in get_tree().get_nodes_in_group("water_tier8"):
				i.disabled = false
...
func _on_close_button_pressed():
	water_talent_tree.visible = false
	save_djinn_data()
	queue_free()
